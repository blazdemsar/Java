...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public void insertUsingQueryAnnotation(int empId, String name, String designation, int salary, boolean insured) {
		try(Session session = sessionFactory.openSession();){
			session.beginTransaction();
			String sqlString = "insert into employee(empId, name, designation, salary, insured) "+
				    " values(:empId, :name, :designation, :salary, :insured)";
			NativeQuery nq = session.createNativeQuery(sqlString);
		
			nq.setParameter("empId", empId);
			nq.setParameter("name", name);
			nq.setParameter("designation", designation);
