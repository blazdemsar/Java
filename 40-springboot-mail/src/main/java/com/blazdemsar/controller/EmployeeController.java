...
import com.blazdemsar.repository.PassportRepository;
import com.blazdemsar.repository.ProjectRepository;
import com.blazdemsar.service.EmployeeService;
import com.blazdemsar.utilities.EmployeeNotFoundException;
...
//	@PreAuthorize(value = "hasAuthority('DBA')")
	...
	/*
	 * @RequestMapping(value="saveEmployee") public String
	 * saveEmployeeForm(@Valid @ModelAttribute Employee employee, BindingResult br,
	 * Model model) { System.out.println(
	 * "@EmployeeController.getEmployeeForm(...)........employee:"+employee);
	 * System.out.println("br.hasErrors(): "+br.hasErrors() ); if(! br.hasErrors())
	 * { employeeService.save(employee); } employee = new Employee();
	 * model.addAttribute("spokenLanguages", spokenLanguages());
	 * model.addAttribute("hobbies", hobbies()); model.addAttribute("employees",
	 * employeeService.findAll()); model.addAttribute("employee", employee); return
	 * "employeeForm"; }
	 */

	// Use of ModelAndView
	/*
	 * @RequestMapping(value="saveEmployee") public ModelAndView
	 * saveEmployeeForm(@Valid @ModelAttribute Employee employee, BindingResult br)
	 * { ModelAndView mv = new ModelAndView("employeeForm"); System.out.println(
	 * "@EmployeeController.getEmployeeForm(...)........employee:"+employee);
	 * System.out.println("br.hasErrors(): "+br.hasErrors() ); if(! br.hasErrors())
	 * { employeeService.save(employee); } employee = new Employee();
	 * mv.addObject("spokenLanguages", spokenLanguages()); mv.addObject("hobbies",
	 * hobbies()); mv.addObject("employees", employeeService.findAll());
	 * 
	...
	public String saveEmployeeForm(@Valid @ModelAttribute Employee employee, BindingResult br, ModelMap modelMap) {
		...
		modelMap.addAttribute("spokenLanguages", spokenLanguages()).addAttribute("hobbies", hobbies());
		...
		if (br.hasErrors()) {
			...
			System.out.println(
					"@EmployeeController.getEmployeeForm(...)....before a call to employeeService.save(employee):"
							+ employee);
	...
	public ResponseEntity<List<Employee>> findAllEmployeesSortedBy(@RequestParam(required = false) String str1,
		...
		if ((str1 != null) && (str2 != null)) {
			return new ResponseEntity<List<Employee>>(
					employeeRepository.findAll(Sort.by(Sort.Direction.DESC, str1, str2)), HttpStatus.ACCEPTED);
		}
		...
		if ((str1 != null) && (str2 == null)) {
			return new ResponseEntity<List<Employee>>(employeeRepository.findAll(Sort.by(Sort.Direction.DESC, str1)),
					HttpStatus.ACCEPTED);
		}
	...
	// we can use @GetMapping, @PostMapping, @PutMapping, @DeleteMapping from spring
	// 4.x, there is no need to mention the http method
	public ResponseEntity<String> insertUsingQueryAnnotation(@RequestParam int empId, @RequestParam String name,
			@RequestParam String job, @RequestParam int salary, @RequestParam boolean insured) {

		employeeService.insertUsingQueryAnnotation(empId, name, job, salary, insured);
		return new ResponseEntity<String>("Employee data saved.", HttpStatus.CREATED);

	}

	// @GetMapping(value="employee/update") // supported by both the browser and
	// Rest Clinet (Postman)
	@PutMapping(value = "employee/update") // Browser does not support Http POST, so @PutMapping does not work with
											// browser, but it works fine with Rest client Postman.
	// we can use @GetMapping, @PostMapping, @PutMapping, @DeleteMapping from spring
	// 4.x, there is no need to mention the http method
	public ResponseEntity<String> updateSalaryByEmpIdUsingQueryAnnotation(@RequestParam int empId,
	...
	// @DeleteMapping(value="employee/delete") // Browser does not support Http
	// POST, so @PutMapping does not work with browser, but it works fine with Rest
	// client Postman.
	// we can use @GetMapping, @PostMapping, @PutMapping, @DeleteMapping from spring
	// 4.x, there is no need to mention the http method
	...
	public ResponseEntity<?> findById(@RequestParam int empId) {
		 ...
		 * ResponseEntity<String>("There is no employee whose id is "+empId+".",
		 * HttpStatus.NOT_FOUND); }else { return new ResponseEntity<Employee>(e,
		 * HttpStatus.FOUND); }
		 */
	}
	...
	public ResponseEntity<?> findByName(@RequestParam String name) {
		...
		/*
		 * // Following code snippet shows how we can handle the Exception
		 * using @ControllerAdvice
		 * 
		 * if(employees.isEmpty()) throw new
		 * EmployeeNotFoundException("There is no employee whose name is \"" +name
		 * +"\""); return new ResponseEntity<List<Employee>>(employees,
		 * HttpStatus.FOUND);
		 * 
		 * 
		 * // Following code snippet shows how we can handle the situation without any
		 * exception being thrown.
		 * 
		 * if(employees.isEmpty()) { return new
		 * ResponseEntity<String>("There is no employee whose name is "+name+".",
		 * HttpStatus.NOT_FOUND); }else { return new
		 * ResponseEntity<List<Employee>>(employees, HttpStatus.FOUND); }
		 */
